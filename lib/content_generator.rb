...
module ContentGenerator
  module Vm
    ...
    def self.size(location, family, size)
      ...
      vm_family = Option::VmFamilies.find { _1.name == family }
      unit_price = BillingRate.from_resource_properties(vm_family.billing_resource_type, vm_family.name, location)["unit_price"].to_f
  ...
  module Postgres
    ...
    def self.size(flavor, location, family, size)
      location = LocationNameConverter.to_internal_name(location)
      size = Option::PostgresSizes.find { _1.display_name == size && _1.flavor == flavor && _1.location == location }
      vm_family = Option::VmFamilies.find { _1.name == family }
      unit_price = BillingRate.from_resource_properties(Option::POSTGRES_BILLING_RESOURCE_TYPES[vm_family.billing_resource_type][:primary], flavor, location)["unit_price"].to_f
