...
class Prog::Postgres::PostgresResourceNexus < Prog::Base
  ...
  label def create_billing_record
    ...
    (postgres_resource.required_standby_count + 1).times do |index|
      billing_record_parts.push({resource_type: index.zero? ? "PostgresVCpu" : "PostgresStandbyVCpu", resource_family: "#{vm_family}-#{postgres_resource.flavor}", amount: representative_server.vm.vcpus})
